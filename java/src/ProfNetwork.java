...
public static int addFriend(String connection_id, ProfNetwork esql){
      ...
      try{
         //not a new user, connection rules apply
            ...
            + " UNION ALL SELECT DISTINCT C2.connectionId FROM CONNECTION_USR C2 WHERE C2.userId = C1.connnectionId"
            + " UNION ALL SELECT DISTINCT C3 connectionId FROM CONNECTION_USR C3 WHERE C3.userId = C2.connectionId"
            + " UNION ALL SELECT DISTINCT C1.userId FROM CONNECTION_USR C1 WHERE C1.connectionId = '%s'"
            + " UNION ALL SELECT DISTINCT C2.userId FROM CONNECTION_USR C2 WHERE C2.connectionId = C1.userId"
            + " UNION ALL SELECT DISTINCT C3.userId FROM CONNECTION_USR C3 WHERE C3.connectionId = C2.userId",usr,usr);
