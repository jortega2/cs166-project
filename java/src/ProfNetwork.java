...
public static void FriendList(ProfNetwork esql){
   try{
      boolean menu = true;
      String friendlist_query = String.format("SELECT ROW_NUMBER() OVER() AS num_row, * FROM (SELECT u.name FROM CONNECTION_USR c, USR u WHERE c.connectionId = '%s' AND u.userId = c.userId UNION SELECT u.name from CONNECTION_USR c, USR u WHERE c.userId = '%s' AND u.userId = c.connectionId)", usr, usr);
